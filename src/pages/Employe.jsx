...
export default function Employe() {
  ...
  useEffect(()=>{
    if(selectedEmp){ loadShifts(); loadPointages() }
  },[selectedEmp])
  ...
  async function loadShifts(){
    ...
    const {data} = await supabase.from('shifts').select('*').eq('employe_id',selectedEmp.id).gte('date',from).lte('date',to)
    setShifts(data||[])
  }

  async function loadPointages(){
    const {data} = await supabase.from('pointages').select('*').eq('employe_id',selectedEmp.id).eq('date',today)
    setPointages(data||[])
  }
    ...
    <div style={{minHeight:'100vh',background:'var(--bg)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:24}}>
      <div style={{width:'100%',maxWidth:400}}>
        <div style={{textAlign:'center',marginBottom:32}}>
          <div style={{width:56,height:56,background:'var(--accent)',borderRadius:14,display:'flex',alignItems:'center',justifyContent:'center',fontSize:28,margin:'0 auto 16px'}}>🍽️</div>
          <div style={{fontSize:24,fontWeight:800}}>RestoPlan</div>
          <div style={{fontSize:14,color:'var(--text2)',marginTop:4}}>Connectez-vous à votre espace</div>
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {employes.map((emp,i)=>{
            const c = COLORS[i%COLORS.length]
            return (
              <button key={emp.id} onClick={()=>setSelectedEmp(emp)} style={{
              ...
              onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.boxShadow='none'}}>
                <div style={{width:44,height:44,borderRadius:'50%',background:c.bg,color:c.color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,fontWeight:800,flexShrink:0}}>{ini(emp.prenom,emp.nom)}</div>
                <div>
                  <div style={{fontSize:15,fontWeight:700,color:'var(--text)'}}>{emp.prenom} {emp.nom}</div>
                  <div style={{fontSize:12,color:'var(--text2)',marginTop:2}}>{emp.role}</div>
                </div>
  ...
  const empIdx = employes.findIndex(e=>e.id===selectedEmp.id)
    ...
    <div style={{maxWidth:430,margin:'0 auto',minHeight:'100vh',background:'var(--bg)',display:'flex',flexDirection:'column',fontFamily:'var(--font)'}}>
      ...
      <div style={{background:'var(--surface)',padding:'16px 20px 0',borderBottom:'1px solid var(--border)'}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
          <div style={{width:38,height:38,borderRadius:'50%',background:empColor.bg,color:empColor.color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:800}}>{ini(selectedEmp.prenom,selectedEmp.nom)}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:16,fontWeight:800}}>Bonjour, {selectedEmp.prenom} 👋</div>
            <div style={{fontSize:12,color:'var(--text2)'}}>{dateStr}</div>
          </div>
          <button onClick={()=>setSelectedEmp(null)} style={{fontSize:12,color:'var(--text3)',background:'var(--bg)',border:'1px solid var(--border)',borderRadius:8,padding:'5px 10px',cursor:'pointer'}}>Changer</button>
        </div>
        ...
        <div style={{flex:1,overflowY:'auto',padding:20,display:'flex',flexDirection:'column',gap:12}}>
          ...
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,padding:16}}>
            ...
            {todayShift ? (
              ...
              <div style={{fontSize:13,color:'var(--text3)',padding:'8px 0'}}>Pas de shift planifié aujourd'hui — bonne journée ! 😊</div>
            )}
          </div>
          ...
          {/* Heures du mois */}
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,padding:16}}>
            <div style={{fontSize:12,fontWeight:700,color:'var(--text2)',marginBottom:10}}>MES HEURES CE MOIS</div>
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              <div style={{display:'flex',justifyContent:'space-between',fontSize:13,padding:'6px 0',borderBottom:'1px solid var(--border)'}}><span style={{color:'var(--text2)'}}>Cette semaine</span><span style={{fontWeight:700}}>{shifts.length * 7}h planifiées</span></div>
              <div style={{display:'flex',justifyContent:'space-between',fontSize:13,padding:'6px 0'}}><span style={{color:'var(--text2)'}}>Dernier pointage</span><span style={{fontWeight:700}}>{todayPointage?`Aujourd'hui ${todayPointage.heure_arrivee?.slice(0,5)}`:'—'}</span></div>
            </div>
          </div>
        </div>
      )}
      ...
      {tab==='planning' && (
        <div style={{flex:1,overflowY:'auto',padding:20,display:'flex',flexDirection:'column',gap:8}}>
          ...
          <div style={{background:'var(--orange-bg)',border:'1px solid #ffd99a',borderRadius:14,padding:'12px 14px',marginTop:4}}>
            <div style={{fontSize:12,fontWeight:700,color:'#8a4a00'}}>📍 Badgeage sur la borne uniquement</div>
            <div style={{fontSize:11,color:'#a06020',marginTop:3}}>Pointez à l'entrée du restaurant sur la tablette — pas depuis cette app</div>
          </div>
        </div>
      )}
      ...
      {tab==='profil' && (
        <div style={{flex:1,overflowY:'auto',padding:20,display:'flex',flexDirection:'column',gap:12}}>
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,padding:20,textAlign:'center'}}>
            <div style={{width:72,height:72,borderRadius:'50%',background:empColor.bg,color:empColor.color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,fontWeight:800,margin:'0 auto 12px'}}>{ini(selectedEmp.prenom,selectedEmp.nom)}</div>
            <div style={{fontSize:18,fontWeight:800}}>{selectedEmp.prenom} {selectedEmp.nom}</div>
            <div style={{fontSize:13,color:'var(--text2)',marginTop:3}}>{selectedEmp.role}</div>
          ...
          <button onClick={()=>setSelectedEmp(null)} style={{width:'100%',padding:13,borderRadius:14,border:'1px solid var(--border)',background:'var(--surface)',color:'var(--red)',fontSize:14,fontWeight:600,cursor:'pointer'}}>Se déconnecter</button>
        </div>
      )}
