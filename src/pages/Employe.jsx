    ...
    <div style={{maxWidth:430,margin:'0 auto',minHeight:'100vh',background:'var(--bg)',display:'flex',flexDirection:'column',fontFamily:'var(--font)'}}>
      ...
      <div style={{background:'var(--surface)',padding:'16px 20px 0',borderBottom:'1px solid var(--border)'}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16}}>
          ...
          <button onClick={deconnexion} style={{fontSize:12,color:'var(--red)',background:'var(--red-bg)',border:'none',borderRadius:8,padding:'5px 10px',cursor:'pointer',fontWeight:600}}>Deconnexion</button>
        </div>
        ...
        <div style={{flex:1,overflowY:'auto',padding:20,display:'flex',flexDirection:'column',gap:12}}>
          <div style={{background:isPresent?'var(--green-bg)':'var(--surface)',border:`1px solid ${isPresent?'#b8e8c8':'var(--border)'}`,borderRadius:16,padding:16}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:14,fontWeight:700,color:isPresent?'#1a6b35':'var(--text)'}}>{isPresent?'Vous etes pointe':isParti?'Journee terminee':"Pas encore pointe"}</div>
          ...
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,padding:16}}>
            ...
            {todayShift ? (
              <div style={{position:'relative',paddingLeft:12}}>
                ...
                <div style={{fontSize:20,fontWeight:800,margin:'4px 0 2px'}}>{todayShift.heure_debut.slice(0,5)} - {todayShift.heure_fin.slice(0,5)}</div>
                {todayShift.heure_debut_2&&<div style={{fontSize:16,fontWeight:700,color:'var(--text2)',marginTop:2}}>puis {todayShift.heure_debut_2.slice(0,5)} - {todayShift.heure_fin_2.slice(0,5)}</div>}
                <span style={{fontSize:11,fontWeight:700,padding:'4px 10px',borderRadius:20,marginTop:6,display:'inline-block',...shiftColors[todayShift.poste]}}>{todayShift.poste[0].toUpperCase()+todayShift.poste.slice(1)}</span>
              </div>
            ) : (
          ...
          {weekDays.map((d,i)=>{
            ...
            return (
              <div key={i} style={{background:'var(--surface)',border:`1px solid ${isToday?'var(--accent)':'var(--border)'}`,borderRadius:14,overflow:'hidden'}}>
                ...
                {sh ? (
                  <div style={{padding:'10px 14px',borderTop:`1px solid ${sc.border}`,background:sc.bg}}>
                    <div style={{fontSize:13,fontWeight:700,color:sc.color}}>{sh.heure_debut.slice(0,5)} - {sh.heure_fin.slice(0,5)}</div>
                    {sh.heure_debut_2&&<div style={{fontSize:12,fontWeight:600,color:sc.color,opacity:.85,marginTop:2}}>puis {sh.heure_debut_2.slice(0,5)} - {sh.heure_fin_2.slice(0,5)}</div>}
                    <div style={{fontSize:11,color:sc.color,opacity:.75,marginTop:4}}>
                      {(()=>{
