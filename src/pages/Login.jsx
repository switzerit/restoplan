...
function QRDisplay() {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:6}}>
      <div style={{background:'white',borderRadius:6,padding:7,opacity:fade?.15:1,transition:fade?'opacity .25s':'none',border:'1px solid #e5e5ea'}}>
...
function RoiCalc({isMobile}) {
  ...
  return (
    <div style={{background:'white',border:'1px solid #e5e5ea',borderRadius:20,padding:isMobile?'24px':'40px',maxWidth:640,margin:'0 auto',boxShadow:'0 2px 24px rgba(0,0,0,.04)'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:20}}>
        {[
          ...
          <div key={i} style={{background:s.bg,border:`1px solid ${s.bc}`,borderRadius:14,padding:'18px 12px',textAlign:'center'}}>
            <div style={{fontSize:isMobile?22:28,fontWeight:900,color:s.c,letterSpacing:'-.03em'}}>{s.v}</div>
            <div style={{fontSize:11,color:s.c,marginTop:5,fontWeight:500,lineHeight:1.4}}>{s.l}</div>
          </div>
        ))}
      </div>
      <div style={{background:'#f0f6ff',border:'1px solid rgba(0,113,227,.15)',borderRadius:12,padding:'14px 20px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
        ...
        <a href="/contact" style={{padding:'9px 18px',borderRadius:9,background:'#0071e3',color:'white',fontSize:13,fontWeight:700,textDecoration:'none',whiteSpace:'nowrap'}}>Obtenir mon devis →</a>
      </div>
    </div>
  )
}
    ...
    <div style={{maxWidth:620,margin:'0 auto'}}>
      {faqs.map((faq,i)=>(
        <div key={i} style={{borderBottom:'1px solid #e5e5ea'}}>
          <button onClick={()=>setOpen(open===i?null:i)} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'19px 0',background:'none',border:'none',cursor:'pointer',textAlign:'left',gap:16}}>
            <span style={{fontSize:15,fontWeight:600,color:'#1d1d1f',lineHeight:1.4}}>{faq.q}</span>
            <span style={{fontSize:20,color:'#0071e3',flexShrink:0,fontWeight:300,transform:open===i?'rotate(45deg)':'none',transition:'transform .2s',display:'inline-block',lineHeight:1,marginTop:-1}}>{open===i?'−':'+'}</span>
          </button>
          {open===i&&<div style={{paddingBottom:18}}><p style={{fontSize:14,color:'#6e6e73',lineHeight:1.8,margin:0}}>{faq.a}</p></div>}
        </div>
      ))}
...
export default function Login() {
  ...
  const W={maxWidth:1080,margin:'0 auto',padding:isMobile?'0 20px':'0 48px'}
  const WM={maxWidth:860,margin:'0 auto',padding:isMobile?'0 20px':'0 48px'}
  ...
  const Nav=()=>(
    <>
      <nav style={{position:'fixed',top:0,left:0,right:0,zIndex:100,height:60,background:'rgba(255,255,255,.97)',backdropFilter:'blur(20px)',borderBottom:`1px solid ${BORDER}`,display:'flex',alignItems:'center',padding:'0 40px'}}>
        <div onClick={()=>goPage('home')} style={{display:'flex',alignItems:'center',gap:8,flex:1,cursor:'pointer',userSelect:'none'}}>
          {LOGO_SM}
          <span style={{fontSize:16,fontWeight:800,color:TEXT,letterSpacing:'-.04em'}}>Kronvo</span>
          <span style={{fontSize:9,fontWeight:700,padding:'2px 7px',borderRadius:20,background:'#e8f2fd',color:A,letterSpacing:'.04em',marginLeft:2}}>BETA</span>
        </div>
        {isMobile?(
          <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={()=>setMenuOpen(!menuOpen)} style={{width:36,height:36,borderRadius:8,border:`1px solid ${BORDER}`,background:SURF,cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:4}}>
              {[0,1,2].map(i=><span key={i} style={{width:14,height:1.5,background:TEXT,borderRadius:1,display:'block'}}></span>)}
            </button>
          </div>
        ):(
          <div style={{display:'flex',gap:1,alignItems:'center'}}>
            {[['Accueil','home'],['Fonctionnalités','fonctionnalites'],['Tarifs','tarifs'],['Contact','contact']].map(([l,id])=>(
              <button key={id} onClick={()=>goPage(id)} style={{padding:'6px 14px',borderRadius:8,border:'none',background:page===id?'#f0f5ff':'transparent',color:page===id?A:TEXT2,fontSize:13,fontWeight:page===id?600:500,cursor:'pointer',transition:'background .15s'}}>{l}</button>
            ))}
            <div style={{width:1,height:16,background:BORDER,margin:'0 12px'}}/>
            <button onClick={()=>setShowLogin(true)} style={{padding:'7px 16px',borderRadius:8,border:`1px solid ${BORDER}`,background:SURF,color:TEXT,fontSize:13,fontWeight:500,cursor:'pointer'}}>Connexion</button>
            <button onClick={()=>goPage('contact')} style={{padding:'7px 16px',borderRadius:8,border:'none',background:A,color:'white',fontSize:13,fontWeight:700,cursor:'pointer',marginLeft:6}}>Demander une démo</button>
          </div>
        )}
      </nav>
      {isMobile&&menuOpen&&(
        <div style={{position:'fixed',top:60,left:0,right:0,zIndex:99,background:SURF,borderBottom:`1px solid ${BORDER}`,boxShadow:'0 8px 32px rgba(0,0,0,.08)',padding:'8px 0 20px'}}>
          {[['Accueil','home'],['Fonctionnalités','fonctionnalites'],['Tarifs','tarifs'],['Contact','contact']].map(([l,id])=>(
  ...
  const Footer=()=>(
    <footer style={{background:DARK,color:'white',padding:isMobile?'44px 20px 32px':'52px 0 32px'}}>
      <div style={W}>
        <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'2fr 1fr 1fr 1fr',gap:isMobile?28:48,marginBottom:32}}>
          <div>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14}}>{LOGO_SM}<span style={{fontSize:15,fontWeight:800,letterSpacing:'-.03em'}}>Kronvo</span></div>
            <div style={{fontSize:13,color:'rgba(255,255,255,.4)',lineHeight:1.8,maxWidth:220}}>La solution de gestion d'équipes pour les professionnels terrain.</div>
            <div style={{fontSize:11,color:'rgba(255,255,255,.2)',marginTop:14}}>Propulsé par <a href="https://switzerit.com" target="_blank" rel="noreferrer" style={{color:'rgba(255,255,255,.4)',textDecoration:'none',fontWeight:600}}>SwitzerIT</a></div>
          </div>
          {[
            ...
            <div key={col.title}>
              <div style={{fontSize:10,fontWeight:700,color:'rgba(255,255,255,.25)',letterSpacing:'.1em',textTransform:'uppercase',marginBottom:14}}>{col.title}</div>
              {col.links.map(([l,p])=>(
                <div key={l} onClick={()=>goPage(p)} style={{fontSize:13,color:'rgba(255,255,255,.4)',marginBottom:10,cursor:'pointer',transition:'color .15s'}}
        ...
        <div style={{borderTop:'1px solid rgba(255,255,255,.06)',paddingTop:20,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
          <div style={{fontSize:11,color:'rgba(255,255,255,.2)'}}>© 2026 Kronvo by SwitzerIT · Suisse</div>
  ...
  const PageHome=()=>(
    <>
      {/* ══ 1. HERO ══ */}
      <section style={{paddingTop:60,background:SURF}}>
        <div style={{...W,padding:isMobile?'56px 20px 48px':'80px 48px 72px',display:isMobile?'block':'grid',gridTemplateColumns:'1fr 1fr',gap:64,alignItems:'center'}}>
          <div>
            <div style={{display:'inline-flex',alignItems:'center',gap:7,padding:'5px 13px',borderRadius:20,background:'#f0f6ff',border:'1px solid rgba(0,113,227,.15)',marginBottom:28}}>
              <span style={{width:7,height:7,borderRadius:'50%',background:'#34c759',boxShadow:'0 0 0 2px rgba(52,199,89,.25)',display:'inline-block'}}></span>
              <span style={{fontSize:12,fontWeight:600,color:A,letterSpacing:'.01em'}}>Badgeage QR · Planning · Présences</span>
            </div>
            <h1 style={{fontSize:isMobile?40:64,fontWeight:900,lineHeight:1.02,margin:'0 0 8px',letterSpacing:'-.06em',color:TEXT}}>
              Le planning qui
            </h1>
            <h1 style={{fontSize:isMobile?40:64,fontWeight:900,lineHeight:1.02,margin:'0 0 24px',letterSpacing:'-.06em',color:A}}>
              se gère tout seul.
            </h1>
            <p style={{fontSize:isMobile?15:18,color:TEXT2,lineHeight:1.8,marginBottom:36,maxWidth:400}}>
            ...
            <div style={{display:'flex',gap:10,flexDirection:isMobile?'column':'row',marginBottom:28}}>
              <button onClick={()=>goPage('contact')} style={{padding:'15px 26px',borderRadius:12,border:'none',background:A,color:'white',fontSize:15,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:9,justifyContent:'center',boxShadow:'0 4px 20px rgba(0,113,227,.3)'}}>
                Demander une démo gratuite <Arr/>
              </button>
              <button onClick={()=>goPage('fonctionnalites')} style={{padding:'15px 20px',borderRadius:12,border:`1.5px solid ${BORDER}`,background:SURF,color:TEXT,fontSize:15,fontWeight:600,cursor:'pointer'}}>
                Voir les fonctionnalités
              </button>
            </div>
            <div style={{display:'flex',gap:20,flexWrap:'wrap',alignItems:'center'}}>
              {['Démo gratuite','Sans engagement','2-4h en place'].map(t=>(
                <div key={t} style={{display:'flex',alignItems:'center',gap:6}}>
                  <Chk size={13}/>
                  <span style={{fontSize:12,color:TEXT3,fontWeight:500}}>{t}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {!isMobile&&(
            <div style={{background:BG,borderRadius:20,padding:18,border:`1px solid ${BORDER}`,boxShadow:'0 8px 40px rgba(0,0,0,.06)'}}>
              <div style={{background:DARK,borderRadius:13,padding:'11px 16px',marginBottom:12,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:12}}>
                {[{v:'8',l:'Présents',bg:'#f0faf3',c:'#1a6b35'},{v:'3',l:'Absents',bg:'#fff2f1',c:'#b02020'},{v:'2',l:'En pause',bg:'#fff8ee',c:'#8a5a00'}].map((s,i)=>(
                  <div key={i} style={{background:s.bg,border:`1px solid ${BORDER}`,borderRadius:10,padding:'12px 8px',textAlign:'center'}}>
              ...
              <div style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:12,padding:'12px',marginBottom:10}}>
                <div style={{fontSize:9,fontWeight:700,color:TEXT3,letterSpacing:'.1em',marginBottom:9}}>ÉQUIPE EN DIRECT</div>
                {[{n:'Sophie Martin',p:'Cuisine',h:'09:02',s:'#34c759'},{n:'Marc Dupont',p:'Salle',h:'09:15',s:'#34c759'},{n:'Julie Bernard',p:'Bar',h:'En pause',s:'#ff9500'},{n:'Thomas Petit',p:'Cuisine',h:'Absent',s:'#e0e0e0'}].map((e,i)=>(
                  <div key={i} style={{display:'flex',alignItems:'center',gap:8,padding:'5px 8px',borderRadius:8,background:i%2===0?BG:'transparent',marginBottom:2}}>
                    <div style={{width:26,height:26,borderRadius:'50%',background:'#e8f2fd',display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,fontWeight:700,color:A,flexShrink:0,position:'relative'}}>
              ...
              <div style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:12,padding:'12px 14px',display:'flex',alignItems:'center',gap:12}}>
                <QRDisplay/>
                <div>
                  <div style={{fontSize:11,fontWeight:700,color:TEXT,marginBottom:3}}>QR Code actif</div>
                  <div style={{fontSize:10,color:TEXT2,lineHeight:1.6}}>Vos employés scannent<br/>depuis leur téléphone</div>
                  <div style={{display:'flex',alignItems:'center',gap:4,marginTop:5}}>
                    <span style={{width:5,height:5,borderRadius:'50%',background:'#34c759',display:'inline-block'}}></span>
                    <span style={{fontSize:9,color:'#34c759',fontWeight:600}}>Sécurisé · Renouvelé auto</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section style={{background:DARK,padding:'0'}}>
        {/* Trust bar */}
        <div style={{borderBottom:'1px solid rgba(255,255,255,.06)',padding:'16px 0'}}>
          <div style={W}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:isMobile?12:32,flexWrap:'wrap'}}>
              <div style={{display:'flex',alignItems:'center',gap:9}}>
                <div style={{display:'flex'}}>
                  {[{bg:'rgba(0,113,227,.3)',t:'SM'},{bg:'rgba(52,199,89,.3)',t:'JD'},{bg:'rgba(255,149,0,.3)',t:'AL'}].map((a,i)=>(
                    <div key={i} style={{width:28,height:28,borderRadius:'50%',background:a.bg,border:'2px solid rgba(255,255,255,.1)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:9,fontWeight:700,color:'rgba(255,255,255,.8)',marginRight:i<2?-7:0,zIndex:3-i}}>
                      {a.t}
                    </div>
                  ))}
                </div>
              ...
              <div style={{width:1,height:20,background:'rgba(255,255,255,.08)'}}></div>
              <div style={{display:'flex',alignItems:'center',gap:6}}>
                ...
                <span style={{fontSize:12,color:'rgba(255,255,255,.35)'}}>/ 5 · support SwitzerIT</span>
              </div>
              {!isMobile&&<>
                <div style={{width:1,height:20,background:'rgba(255,255,255,.08)'}}></div>
                <span style={{fontSize:12,color:'rgba(255,255,255,.35)'}}>Restaurants · Hôtels · Cliniques · Garages · BTP & plus</span>
              </>}
            </div>
          </div>
        </div>
        ...
        <div style={{padding:'36px 0'}}>
          <div style={WM}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)'}}>
              {[
                {v:'2-4h',l:'mise en place',c:'#60a5fa',sub:'clé en main'},
                {v:'30s',l:'par badgeage',c:'white',sub:'scan QR code'},
                {v:'8+',l:'secteurs',c:'white',sub:'couverts'},
                {v:'100%',l:'mobile',c:'white',sub:'iPhone & Android'},
              ].map((s,i)=>(
                <div key={i} style={{textAlign:'center',borderRight:i<3?'1px solid rgba(255,255,255,.06)':'none',padding:isMobile?'10px 6px':'12px 24px'}}>
                  <div style={{fontSize:isMobile?26:40,fontWeight:900,color:s.c,letterSpacing:'-.05em',lineHeight:1}}>{s.v}</div>
                  <div style={{fontSize:12,color:'rgba(255,255,255,.5)',marginTop:5,fontWeight:500}}>{s.l}</div>
                  <div style={{fontSize:10,color:'rgba(255,255,255,.25)',marginTop:3}}>{s.sub}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ══ 3. POURQUOI KRONVO — 3 problèmes résolus ══ */}
      <section style={{background:SURF,padding:isMobile?'64px 20px':'88px 0'}}>
        <div style={WM}>
          <div style={{textAlign:'center',marginBottom:52}}>
            <div style={{fontSize:11,fontWeight:700,color:A,letterSpacing:'.12em',textTransform:'uppercase',marginBottom:14}}>Le problème</div>
            <h2 style={{fontSize:isMobile?28:44,fontWeight:900,color:TEXT,letterSpacing:'-.05em',marginBottom:14,lineHeight:1.06}}>
              Vous perdez des heures<br/>sur des tâches inutiles.
            </h2>
            <p style={{fontSize:16,color:TEXT2,lineHeight:1.75,maxWidth:480,margin:'0 auto'}}>Feuilles papier, appels incessants, erreurs de paie — tout ça a un coût réel. Kronvo les élimine.</p>
          </div>
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:14}}>
            {[
              {
                bg:'#fff2f1',border:'#ffd0d0',ic:'#b02020',
                icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#b02020" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/></svg>,
                title:'Feuilles de présence',
                desc:'Perdues, illisibles, recopiées à la main chaque semaine. Un vrai cauchemar pour la paie de fin de mois.',
                sol:'Badgeage QR en 2 secondes',solc:'#1a6b35'
              },
              {
                bg:'#fff8ee',border:'#ffd9a0',ic:'#8a5a00',
                icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#8a5a00" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.14 14a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.05 3h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.09 10a16 16 0 0 0 6.29 6.29l1.35-1.35a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>,
                title:'Appels pour les absences',
                desc:'"Qui est là aujourd\'hui ?" — une question répétée chaque matin qui vous fait perdre un temps précieux.',
                sol:'Tableau de bord en temps réel',solc:'#1a6b35'
              },
              {
                bg:'#f0f0fc',border:'#d0cff8',ic:'#3a3880',
                icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#3a3880" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>,
                ...
                sol:'Export PDF automatique',solc:'#1a6b35'
              },
            ].map((c,i)=>(
              <div key={i} style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:18,padding:'28px',overflow:'hidden',position:'relative'}}>
                <div style={{position:'absolute',top:0,left:0,right:0,height:3,background:c.border,borderRadius:'18px 18px 0 0'}}></div>
                <div style={{width:48,height:48,background:c.bg,borderRadius:13,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:18}}>{c.icon}</div>
                <div style={{fontSize:16,fontWeight:700,color:TEXT,marginBottom:10,letterSpacing:'-.01em'}}>{c.title}</div>
                <div style={{fontSize:14,color:TEXT2,lineHeight:1.7,marginBottom:18}}>{c.desc}</div>
                <div style={{display:'flex',alignItems:'center',gap:8,padding:'11px 14px',background:'#f0faf3',border:'1px solid #b8e8c8',borderRadius:10}}>
                  <Chk size={14} color="#34c759"/>
                  <span style={{fontSize:13,fontWeight:600,color:c.solc}}>Kronvo — {c.sol}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ══ 4. FONCTIONNALITÉS ══ */}
      <section style={{background:BG,borderTop:`1px solid ${BORDER}`,padding:isMobile?'64px 20px':'88px 0'}}>
        <div style={W}>
          <div style={{textAlign:'center',marginBottom:52}}>
            ...
            <h2 style={{fontSize:isMobile?28:44,fontWeight:900,color:TEXT,letterSpacing:'-.05em',marginBottom:14,lineHeight:1.06}}>Tout ce dont vous avez besoin.</h2>
            <p style={{fontSize:16,color:TEXT2,maxWidth:460,margin:'0 auto',lineHeight:1.7}}>Une solution complète pour les équipes terrain. Ça marche dès le premier jour, sans formation technique.</p>
          </div>
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:2,background:BORDER,borderRadius:18,overflow:'hidden',marginBottom:24}}>
            {[
              ...
              {icon:'📷',title:'Badgeage QR Code',desc:'QR dynamique renouvelé toutes les 30s. Scan depuis le téléphone. Borne tablette disponible pour l\'entrée.',bg:'#f0f0fc'},
              {icon:'👥',title:'Présences en direct',desc:'Visualisez qui est là maintenant. Heures prévues vs heures pointées. Écarts calculés automatiquement.',bg:'#f0faf3'},
              ...
              {icon:'📄',title:'Export PDF paie',desc:'Rapports de présence générés en un clic. Par employé, par période. Prêt pour votre comptable.',bg:'#fff2f1'},
              ...
              <div key={i} style={{background:SURF,padding:'28px 24px',transition:'background .2s'}}
              onMouseEnter={e=>{e.currentTarget.style.background='#f8faff'}}
              onMouseLeave={e=>{e.currentTarget.style.background=SURF}}>
                <div style={{width:44,height:44,borderRadius:12,background:f.bg,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,marginBottom:16}}>{f.icon}</div>
      ...
      {/* ══ 5. SECTEURS ══ */}
      <section style={{background:SURF,borderTop:`1px solid ${BORDER}`,padding:isMobile?'64px 20px':'88px 0'}}>
        <div style={WM}>
          <div style={{display:isMobile?'block':'grid',gridTemplateColumns:'1fr 1.6fr',gap:64,alignItems:'start'}}>
            <div style={{marginBottom:isMobile?32:0}}>
              ...
              <h2 style={{fontSize:isMobile?28:38,fontWeight:900,color:TEXT,letterSpacing:'-.05em',marginBottom:16,lineHeight:1.06}}>
                Votre secteur,<br/>vos postes.
              </h2>
              <p style={{fontSize:15,color:TEXT2,lineHeight:1.75,marginBottom:24}}>
                Kronvo s'adapte à votre activité avec les termes exacts que vous utilisez au quotidien. Pas de configuration générique.
              </p>
              <button onClick={()=>goPage('contact')} style={{padding:'12px 22px',borderRadius:10,border:'none',background:A,color:'white',fontSize:14,fontWeight:700,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:8}}>
                Voir si mon secteur est couvert <Arr size={14}/>
              </button>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              {[
                ...
                <div key={i} style={{background:s.bg,borderRadius:14,padding:'18px 16px',border:'1px solid rgba(0,0,0,.04)'}}>
                  <div style={{fontSize:13,fontWeight:700,color:s.c,marginBottom:6}}>{s.title}</div>
      ...
      <section style={{background:BG,borderTop:`1px solid ${BORDER}`,padding:isMobile?'64px 20px':'88px 0'}}>
        <div style={W}>
          <div style={{textAlign:'center',marginBottom:48}}>
            ...
            <h2 style={{fontSize:isMobile?28:44,fontWeight:900,color:TEXT,letterSpacing:'-.05em',marginBottom:14,lineHeight:1.06}}>Combien vous coûte<br/>l'ancien système ?</h2>
            <p style={{fontSize:16,color:TEXT2,maxWidth:420,margin:'0 auto',lineHeight:1.7}}>Calculez ce que vous perdez chaque mois avec les feuilles papier et les appels inutiles.</p>
          </div>
        ...
        <div style={W}>
          <div style={{display:isMobile?'block':'grid',gridTemplateColumns:'1fr 1fr',gap:80,alignItems:'start'}}>
            <div>
              ...
              <h2 style={{fontSize:isMobile?26:36,fontWeight:900,color:TEXT,letterSpacing:'-.04em',marginBottom:8,lineHeight:1.1}}>Questions fréquentes</h2>
              <p style={{fontSize:15,color:TEXT2,marginBottom:36,lineHeight:1.7}}>Tout ce que vous voulez savoir avant de commencer.</p>
              <FaqSection/>
            </div>
            <div style={{marginTop:isMobile?48:0}}>
              <div style={{background:DARK,borderRadius:20,padding:'36px',marginBottom:14}}>
                <div style={{display:'inline-flex',alignItems:'center',gap:7,padding:'5px 13px',borderRadius:20,background:'rgba(96,165,250,.12)',border:'1px solid rgba(96,165,250,.2)',marginBottom:22}}>
                ...
                <h3 style={{fontSize:isMobile?24:30,fontWeight:900,color:'white',letterSpacing:'-.04em',marginBottom:14,lineHeight:1.1}}>
                  Prêt à simplifier<br/>la gestion de votre équipe ?
                </h3>
                <p style={{fontSize:14,color:'rgba(255,255,255,.45)',marginBottom:28,lineHeight:1.7}}>
                  Démo gratuite via Teams. Notre équipe configure tout pour vous, sans engagement.
                </p>
                <button onClick={()=>goPage('contact')} style={{width:'100%',padding:'15px',borderRadius:12,border:'none',background:A,color:'white',fontSize:15,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:9,justifyContent:'center',marginBottom:10,boxShadow:'0 4px 20px rgba(0,113,227,.4)'}}>
                  Réserver ma démo gratuite <Arr/>
                </button>
                <button onClick={()=>setShowLogin(true)} style={{width:'100%',padding:'13px',borderRadius:12,border:'1px solid rgba(255,255,255,.1)',background:'transparent',color:'rgba(255,255,255,.5)',fontSize:14,fontWeight:500,cursor:'pointer'}}>
                  Déjà client — Se connecter
                </button>
                <div style={{fontSize:11,color:'rgba(255,255,255,.2)',textAlign:'center',marginTop:14}}>Sans carte bancaire · Sans engagement · Réponse sous 24h</div>
              </div>
              <div style={{background:BG,border:`1px solid ${BORDER}`,borderRadius:14,padding:'20px 22px'}}>
                ...
                {[
                  ...
                  {e:'🎓',t:'Formation incluse',d:'Gérants et équipes formés.'},
                  ...
                  <div key={i} style={{display:'flex',gap:12,alignItems:'flex-start',marginBottom:i<3?14:0}}>
                    <div style={{width:32,height:32,borderRadius:9,background:BG,border:`1px solid ${BORDER}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,flexShrink:0}}>{s.e}</div>
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:TEXT,marginBottom:2}}>{s.t}</div>
                      <div style={{fontSize:12,color:TEXT2}}>{s.d}</div>
                    </div>
                  </div>
                ))}
              </div>
  ...
  const PageFeatures=()=>(
    <div style={{paddingTop:60,minHeight:'100vh',background:BG}}>
      <div style={{...W,padding:isMobile?'44px 20px':'60px 48px'}}>
        <div style={{textAlign:'center',marginBottom:48}}>
          <div style={{fontSize:11,fontWeight:700,color:A,letterSpacing:'.12em',textTransform:'uppercase',marginBottom:14}}>Fonctionnalités</div>
          <h1 style={{fontSize:isMobile?30:48,fontWeight:900,color:TEXT,letterSpacing:'-.05em',marginBottom:14}}>Tout ce que Kronvo peut faire</h1>
          <p style={{fontSize:16,color:TEXT2,maxWidth:500,margin:'0 auto',lineHeight:1.7}}>Une solution pensée pour les équipes terrain. Sans complexité inutile.</p>
        </div>
        {[
          ...
          <div key={i} style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:16,padding:isMobile?'22px':'28px',marginBottom:10}}>
            <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:18}}>
              <div style={{width:50,height:50,background:f.color,borderRadius:14,display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0}}>{f.icon}</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(auto-fit,minmax(200px,1fr))',gap:8}}>
              {f.items.map((item,j)=>(
                <div key={j} style={{display:'flex',alignItems:'flex-start',gap:9,padding:'10px 13px',background:f.color,borderRadius:9}}>
        ...
        <div style={{background:'#f0f6ff',border:'1px solid rgba(0,113,227,.12)',borderRadius:16,padding:'30px',textAlign:'center',marginTop:10}}>
          <div style={{fontSize:20,fontWeight:800,color:TEXT,marginBottom:10,letterSpacing:'-.02em'}}>Fonctionnalités à venir</div>
    ...
    <div style={{paddingTop:60,minHeight:'100vh',background:BG}}>
      <div style={{...W,padding:isMobile?'44px 20px':'60px 48px'}}>
        <div style={{textAlign:'center',marginBottom:48}}>
          ...
          <h1 style={{fontSize:isMobile?30:48,fontWeight:900,color:TEXT,letterSpacing:'-.05em',marginBottom:14}}>Tarification sur mesure</h1>
          <p style={{fontSize:15,color:TEXT2,maxWidth:480,margin:'0 auto',lineHeight:1.7}}>Chaque entreprise est différente. Devis personnalisé selon vos besoins réels. Démo gratuite incluse.</p>
        </div>
        <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:14,marginBottom:20}}>
          {[
            ...
            <div key={i} style={{background:SURF,border:plan.featured?`2px solid ${A}`:`1px solid ${BORDER}`,borderRadius:18,padding:'26px',display:'flex',flexDirection:'column',position:'relative',boxShadow:plan.featured?'0 8px 40px rgba(0,113,227,.1)':'none'}}>
              {plan.featured&&<div style={{position:'absolute',top:-13,left:'50%',transform:'translateX(-50%)',padding:'4px 16px',borderRadius:20,background:A,color:'white',fontSize:10,fontWeight:700,whiteSpace:'nowrap',letterSpacing:'.04em'}}>RECOMMANDÉ</div>}
              <div style={{width:42,height:42,background:plan.color,borderRadius:12,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,marginBottom:14}}>{plan.icon}</div>
              <div style={{fontSize:17,fontWeight:800,color:TEXT,marginBottom:6,letterSpacing:'-.01em'}}>{plan.name}</div>
              ...
              <div style={{padding:'9px 14px',background:plan.color,borderRadius:9,fontSize:13,fontWeight:700,color:plan.ic,textAlign:'center',marginBottom:13}}>{plan.tag}</div>
        ...
        <div style={{background:'#f0f6ff',border:'1px solid rgba(0,113,227,.15)',borderRadius:14,padding:'24px',display:'flex',alignItems:isMobile?'flex-start':'center',gap:16,flexDirection:isMobile?'column':'row'}}>
          <div style={{fontSize:32,flexShrink:0}}>📹</div>
          <div style={{flex:1}}>
            <div style={{fontSize:16,fontWeight:800,color:TEXT,marginBottom:5}}>Démo gratuite via Teams</div>
    ...
    <div style={{paddingTop:60,minHeight:'100vh',background:BG}}>
      <div style={{...W,padding:isMobile?'44px 20px':'60px 48px'}}>
        <div style={{textAlign:'center',marginBottom:44}}>
          ...
          <h1 style={{fontSize:isMobile?28:44,fontWeight:900,color:TEXT,letterSpacing:'-.05em',marginBottom:14}}>Parlons de votre projet</h1>
          ...
          <div style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:18,padding:'28px'}}>
            {contactSent?(
              <div style={{textAlign:'center',padding:'44px 20px'}}>
                <div style={{width:64,height:64,borderRadius:'50%',background:'#f0faf3',border:'1px solid #b8e8c8',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 20px'}}>
                  <Chk size={28} color="#34c759"/>
                </div>
                <div style={{fontSize:22,fontWeight:800,color:TEXT,marginBottom:10}}>Demande envoyée !</div>
                <div style={{fontSize:14,color:TEXT2,lineHeight:1.75,marginBottom:20}}>Notre équipe vous contactera sous 24h pour organiser la démo Teams.</div>
                <button onClick={()=>{setContactSent(false);setContactForm({nom:'',email:'',entreprise:'',secteur:'',message:''})}} style={{padding:'10px 22px',borderRadius:9,border:`1px solid ${BORDER}`,background:BG,color:TEXT,fontSize:13,fontWeight:600,cursor:'pointer'}}>Nouvelle demande</button>
              </div>
            ):(
              <>
                <div style={{fontSize:17,fontWeight:800,color:TEXT,marginBottom:5,letterSpacing:'-.01em'}}>Demander une démo Teams</div>
                <div style={{fontSize:12,color:TEXT2,marginBottom:22}}>Gratuite · 30 min · Sans engagement</div>
                ...
                style={{width:'100%',height:52,borderRadius:12,border:'none',background:A,color:'white',fontSize:15,fontWeight:700,cursor:'pointer',boxShadow:'0 4px 16px rgba(0,113,227,.2)'}}>
                  Envoyer ma demande →
                </button>
                <div style={{fontSize:11,color:TEXT3,textAlign:'center',marginTop:10}}>Réponse sous 24h · Démo Teams offerte</div>
              </>
            )}
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            <div style={{background:DARK,borderRadius:16,padding:'24px',color:'white'}}>
              <div style={{fontSize:32,marginBottom:14}}>📹</div>
              <div style={{fontSize:15,fontWeight:700,marginBottom:8}}>Démo Teams gratuite</div>
              <div style={{fontSize:13,color:'rgba(255,255,255,.45)',lineHeight:1.7}}>30 min pour voir Kronvo en action adapté à votre secteur. Repartez avec un devis personnalisé.</div>
            </div>
            {[{icon:'🚀',title:'Mis en place par SwitzerIT',desc:"Notre équipe s'occupe de tout."},{icon:'🇨🇭',title:'Facturation en CHF',desc:'Sans conversion ni frais cachés.'}].map((info,i)=>(
              <div key={i} style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:13,padding:'18px',display:'flex',gap:13,alignItems:'flex-start'}}>
                <div style={{fontSize:22,flexShrink:0}}>{info.icon}</div>
                <div>
                  <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:4}}>{info.title}</div>
            ...
            <div style={{background:'#f0f6ff',border:'1px solid rgba(0,113,227,.15)',borderRadius:13,padding:'18px'}}>
              ...
              <div style={{fontSize:12,color:TEXT2,marginBottom:12}}>Accédez à votre espace gérant.</div>
              <button onClick={()=>setShowLogin(true)} style={{padding:'9px 18px',borderRadius:8,border:'none',background:A,color:'white',fontSize:12,fontWeight:700,cursor:'pointer'}}>Se connecter →</button>
            </div>
            <div style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:13,padding:'18px'}}>
              <div style={{fontSize:12,fontWeight:700,color:TEXT,marginBottom:10}}>Contact direct</div>
              <div style={{fontSize:12,color:TEXT2,marginBottom:5}}>📧 contact@switzerit.com</div>
              <div style={{fontSize:12,color:TEXT2,marginBottom:5}}>🌐 switzerit.com</div>
  ...
  const PageLegal=()=>{
    ...
    return (
      <div style={{paddingTop:60,minHeight:'100vh',background:BG}}>
        <div style={{...W,padding:isMobile?'28px 20px':'60px 48px'}}>
          {isMobile?(
            <>
              <div style={{display:'flex',gap:8,marginBottom:22,overflowX:'auto',paddingBottom:4}}>
                {[['cgu','CGU'],['confidentialite','Confidentialité'],['rgpd','RGPD'],['cookies','Cookies']].map(([id,label])=>(
                  <button key={id} onClick={()=>setLegalSection(id)} style={{padding:'7px 15px',borderRadius:20,border:'none',background:legalSection===id?A:'transparent',color:legalSection===id?'white':TEXT2,fontSize:13,fontWeight:600,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}>{label}</button>
                ))}
              </div>
              <div style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:14,padding:'24px'}}>
                ...
                <div style={{marginTop:22,padding:'13px 16px',background:'#f0f6ff',borderRadius:10,fontSize:13,color:A}}><strong>Des questions ?</strong> <a href="mailto:contact@switzerit.com" style={{color:A}}>contact@switzerit.com</a></div>
              </div>
            </>
          ):(
            <div style={{display:'grid',gridTemplateColumns:'190px 1fr',gap:26,alignItems:'start'}}>
              <div style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:13,padding:'17px 13px',position:'sticky',top:74}}>
                <div style={{fontSize:10,fontWeight:700,color:TEXT3,letterSpacing:'.1em',textTransform:'uppercase',marginBottom:13}}>Légal</div>
                {[['cgu','CGU'],['confidentialite','Confidentialité'],['rgpd','RGPD'],['cookies','Cookies']].map(([id,label])=>(
                  <button key={id} onClick={()=>setLegalSection(id)} style={{width:'100%',padding:'9px 12px',borderRadius:8,border:'none',background:legalSection===id?'#f0f6ff':'transparent',color:legalSection===id?A:TEXT2,fontSize:13,fontWeight:legalSection===id?600:400,cursor:'pointer',textAlign:'left',marginBottom:2,display:'block'}}>{label}</button>
                ))}
              </div>
              <div style={{background:SURF,border:`1px solid ${BORDER}`,borderRadius:16,padding:'36px'}}>
                ...
                {s.content.map((block,i)=>(<div key={i} style={{marginBottom:24}}><h2 style={{fontSize:15,fontWeight:700,color:TEXT,marginBottom:8}}>{block.h}</h2><p style={{fontSize:14,color:TEXT2,lineHeight:1.8,margin:0}}>{block.t}</p></div>))}
                <div style={{marginTop:32,padding:'16px 20px',background:'#f0f6ff',borderRadius:12,fontSize:13,color:A}}><strong>Des questions ?</strong> <a href="mailto:contact@switzerit.com" style={{color:A}}>contact@switzerit.com</a></div>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div style={{minHeight:'100vh',background:BG,fontFamily:'var(--font)',color:TEXT}}>
      ...
      <Footer/>
      {showLogin&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.55)',backdropFilter:'blur(12px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:16}}>
          <div style={{background:SURF,borderRadius:20,padding:isMobile?'24px 20px':34,width:'100%',maxWidth:360,boxShadow:'0 28px 72px rgba(0,0,0,.18)',border:`1px solid ${BORDER}`,position:'relative'}}>
            ...
            <div style={{textAlign:'center',marginBottom:22}}>
              <div style={{display:'flex',justifyContent:'center',marginBottom:13}}>{LOGO_SM}</div>
              <div style={{fontSize:20,fontWeight:800,color:TEXT,letterSpacing:'-.03em'}}>Connexion</div>
              <div style={{fontSize:12,color:TEXT2,marginTop:4}}>Accédez à votre espace Kronvo</div>
            </div>
            <form onSubmit={handleLogin}>
              ...
              {error&&<div style={{padding:'10px 13px',background:'#fff2f1',border:'1px solid rgba(255,59,48,.2)',borderRadius:9,fontSize:13,color:'#b02020',marginBottom:14,fontWeight:600}}>{error}</div>}
