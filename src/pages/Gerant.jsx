  ...
  async function saveCorrection(){
    ...
    if(!p){
      await supabase.from('pointages').insert({
        employe_id:correctModal.empId,date:today,
        heure_arrivee:correctForm.heure_arrivee||null,
        heure_depart:correctForm.heure_depart||null,
    ...
    <div style={{display:'flex',height:'100vh',fontFamily:'var(--font)',overflow:'hidden'}}>
      ...
      <div style={{width:220,background:'var(--surface)',borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',padding:'18px 10px',flexShrink:0}}>
        ...
        <div style={{marginBottom:20}}>
          ...
          {/* SWITCHER RESTAURANT */}
          <button onClick={()=>setShowRestoSwitch(!showRestoSwitch)} style={{
            width:'100%',display:'flex',alignItems:'center',gap:8,padding:'10px 10px',
            borderRadius:10,border:'1.5px solid var(--border2)',background:'var(--bg)',
          ...
          {showRestoSwitch && (
            <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,marginTop:4,overflow:'hidden',boxShadow:'0 4px 16px rgba(0,0,0,.08)'}}>
              {restaurants.map(r=>(
                <button key={r.id} onClick={()=>{setCurrentResto(r);setShowRestoSwitch(false)}} style={{
                  width:'100%',padding:'10px 12px',border:'none',background:r.id===currentResto.id?'var(--accent-bg)':'transparent',
                  cursor:'pointer',textAlign:'left',fontSize:13,fontWeight:600,
                  color:r.id===currentResto.id?'var(--accent)':'var(--text)',
                  borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:8
                }}>
                  {r.id===currentResto.id && <span style={{color:'var(--accent)'}}>✓</span>}
                  <span>{r.nom}</span>
                </button>
              ))}
              <button onClick={()=>{setShowRestoSwitch(false);setRestoModal(true)}} style={{
                width:'100%',padding:'10px 12px',border:'none',background:'transparent',
                cursor:'pointer',textAlign:'left',fontSize:13,fontWeight:600,color:'var(--accent)',display:'flex',alignItems:'center',gap:6
	  ...
	  {id:'parametres',icon:'⚙️',label:'Paramètres'},
          ...
          <button key={item.id} onClick={()=>setView(item.id)} style={{
            ...
            color:view===item.id?'var(--accent)':'var(--text2)',marginBottom:2
          }}>
            ...
            <div style={{background:'var(--surface)',borderRadius:14,border:'1px solid var(--border)',overflow:'hidden'}}>
              ...
              {employes.length===0 && (
                <div style={{padding:40,textAlign:'center',color:'var(--text3)',fontSize:14}}>
                  Aucun employé dans ce restaurant — <button onClick={()=>setEmpModal(true)} style={{color:'var(--accent)',background:'none',border:'none',cursor:'pointer',fontWeight:600,fontSize:14}}>ajouter un employé</button>
                </div>
              )}
              {employes.map((emp,ei)=>{
              ...
              {employes.map((emp,i)=>{
                ...
                return (
                  <div key={emp.id} style={{display:'flex',alignItems:'center',gap:11,padding:'11px 14px',background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10}}>
                    ...
                    {/* BOUTON CORRECTION */}
                    <button onClick={()=>openCorrection(emp)} style={{
                      padding:'6px 10px',borderRadius:8,border:'1px solid var(--border2)',
                      background:'var(--bg)',color:'var(--text2)',fontSize:11,fontWeight:600,cursor:'pointer'
                    }}>✏️ Corriger</button>
                  </div>
                )
              })}
    ...
    <div style={{maxWidth:500}}>
      <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,overflow:'hidden',marginBottom:16}}>
        ...
        <div style={{padding:'20px'}}>
          ...
          <div style={{display:'flex',gap:8}}>
            <input type='number' maxLength={4} placeholder='1234' id='new-pin-input'
            style={{flex:1,padding:'10px 14px',borderRadius:10,border:'1.5px solid var(--border2)',background:'var(--bg)',fontSize:18,color:'var(--text)',outline:'none',textAlign:'center'}}/>
            <button onClick={async()=>{
              const val=document.getElementById('new-pin-input').value.slice(0,4)
              if(val.length!==4||isNaN(val)){showToast('Le PIN doit être 4 chiffres');return}
            ...
            }} style={{padding:'10px 18px',borderRadius:10,border:'none',background:'var(--accent)',color:'white',fontSize:13,fontWeight:700,cursor:'pointer'}}>
              Modifier
            </button>
          </div>
      ...
      <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,overflow:'hidden'}}>
        ...
        <div style={{padding:'20px',display:'flex',flexDirection:'column',gap:12}}>
          <div>
            ...
            <input defaultValue={currentResto.nom} id='resto-nom'
            style={{width:'100%',padding:'10px 14px',borderRadius:10,border:'1.5px solid var(--border2)',background:'var(--bg)',fontSize:13,color:'var(--text)',outline:'none'}}/>
          </div>
          <div>
            <label style={{display:'block',fontSize:11,fontWeight:700,color:'var(--text2)',marginBottom:6}}>Adresse</label>
            <input defaultValue={currentResto.adresse||''} id='resto-adresse'
            style={{width:'100%',padding:'10px 14px',borderRadius:10,border:'1.5px solid var(--border2)',background:'var(--bg)',fontSize:13,color:'var(--text)',outline:'none'}}/>
          </div>
          <button onClick={async()=>{
          ...
          }} style={{height:40,borderRadius:10,border:'none',background:'var(--accent)',color:'white',fontSize:13,fontWeight:700,cursor:'pointer'}}>
            Enregistrer
          </button>
        </div>
      </div>
