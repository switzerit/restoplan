...
export default function Borne() {
  ...
  async function loadEmployes() {
    const { data } = await supabase.from('employes').select('*').eq('actif', true).order('prenom')
    setEmployes(data || [])
  }

  async function loadPointages() {
    const { data } = await supabase.from('pointages').select('*').eq('date', today)
    const map = {}
  ...
  async function doBadge(emp) {
    ...
    const t = nowStr()
    if (!p) {
      await supabase.from('pointages').insert({ employe_id: emp.id, date: today, heure_arrivee: t })
  ...
  async function forceArrivee(empId) {
    ...
    if (!p) await supabase.from('pointages').insert({ employe_id: empId, date: today, heure_arrivee: t })
    loadPointages()
  }
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',background:'var(--bg)'}}>
      ...
      <div style={{background:'var(--surface)',borderBottom:'1px solid var(--border)',padding:'14px 28px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div>
          <div style={{fontSize:18,fontWeight:800}}>Le Bistrot du Port</div>
          ...
          return (
            <div key={emp.id} onClick={() => setSelected(emp)} style={{
              ...
              <div style={{
                ...
                background: present ? 'rgba(52,199,89,.14)' : 'var(--bg)',
                color: present ? '#1a6b35' : 'var(--text3)'
              }}>
                <div style={{width:6,height:6,borderRadius:'50%',background: present ? 'var(--green)' : 'var(--text3)'}}></div>
                {present ? 'Présent' : 'Pas encore arrivé'}
              </div>
              {present && p.heure_arrivee && (
                <div style={{fontSize:11,color:'#1a7a3a',fontWeight:600}}>Arrivée à {p.heure_arrivee.slice(0,5)}</div>
              )}
            </div>
          )
        })}
      ...
      <div style={{background:'var(--surface)',borderTop:'1px solid var(--border)',padding:'12px 28px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <button onClick={() => setShowPin(true)} style={{
          fontSize:13,fontWeight:600,color:'var(--text2)',background:'#f5f5f7',
          border:'1px solid var(--border2)',borderRadius:10,padding:'8px 16px',display:'flex',alignItems:'center',gap:6
        }}>🔒 Accès gérant</button>
      </div>
        ...
        <div onClick={() => setSelected(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.3)',backdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100}}>
          <div onClick={e => e.stopPropagation()} style={{background:'var(--surface)',borderRadius:28,padding:'30px 26px',width:330,textAlign:'center',boxShadow:'0 12px 50px rgba(0,0,0,.18)'}}>
            {(() => {
              ...
              const present = p && p.heure_arrivee && !p.heure_depart
              return <>
                <div style={{width:78,height:78,borderRadius:'50%',background:c.bg,color:c.color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:27,fontWeight:800,margin:'0 auto 14px'}}>
                ...
                <div style={{fontSize:14,color:'var(--text2)',margin:'18px 0 6px'}}>
                  {present ? `Arrivé à ${p.heure_arrivee.slice(0,5)} — Enregistrer votre départ ?` : 'Enregistrer votre arrivée ?'}
                </div>
                <div style={{fontSize:38,fontWeight:300,letterSpacing:-1}}>{clock}</div>
                <div style={{display:'flex',gap:10,marginTop:16}}>
                  <button onClick={() => setSelected(null)} style={{flex:1,height:50,borderRadius:14,border:'none',background:'#f5f5f7',color:'var(--text2)',fontSize:14,fontWeight:700}}>Annuler</button>
                  <button onClick={() => doBadge(selected)} style={{
                    flex:1,height:50,borderRadius:14,border: present ? '2px solid rgba(255,59,48,.2)' : 'none',
                    background: present ? 'var(--red-bg)' : 'var(--green)',
                    color: present ? 'var(--red)' : 'white',fontSize:14,fontWeight:700
                  }}>{present ? 'Départ' : 'Arrivée'}</button>
                </div>
              </>
            })()}
          </div>
      ...
      {showPin && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.3)',backdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100}}>
          <div style={{background:'var(--surface)',borderRadius:28,padding:'30px 26px',width:340,textAlign:'center',boxShadow:'0 12px 50px rgba(0,0,0,.18)'}}>
            ...
            <div style={{fontSize:13,color:'var(--text2)',marginBottom:24}}>Entrez votre code PIN à 4 chiffres</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
              {['1','2','3','4','5','6','7','8','9','','0','⌫'].map((k,i) => (
                <button key={i} onClick={() => k === '⌫' ? setPin(p => p.slice(0,-1)) : k ? pinKey(k) : null} style={{
                  height:60,borderRadius:14,border: k ? '1.5px solid var(--border)' : 'none',
                  background: k ? 'var(--surface)' : 'transparent',
                  fontSize: k==='⌫' ? 16 : 22,fontWeight:600,color:'var(--text)',
                  cursor: k ? 'pointer' : 'default'
                }}>{k}</button>
              ))}
            </div>
            <button onClick={() => { setShowPin(false); setPin(''); setPinError(false) }} style={{marginTop:14,width:'100%',padding:11,borderRadius:12,border:'none',background:'transparent',color:'var(--text2)',fontSize:14,fontWeight:600}}>Annuler</button>
          </div>
        </div>
      )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.3)',backdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100}}>
          <div style={{background:'var(--surface)',borderRadius:28,width:520,boxShadow:'0 12px 60px rgba(0,0,0,.2)',overflow:'hidden',maxHeight:'90vh',display:'flex',flexDirection:'column'}}>
            <div style={{padding:'20px 24px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:12}}>
              ...
              <div style={{fontSize:17,fontWeight:800,flex:1}}>Panneau gérant</div>
              <button onClick={() => setShowAdmin(false)} style={{width:32,height:32,borderRadius:'50%',border:'none',background:'var(--bg)',color:'var(--text2)',fontSize:18}}>×</button>
            </div>
              ...
              {employes.map((emp, i) => {
                ...
                return (
                  <div key={emp.id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:'var(--bg)',borderRadius:14,marginBottom:7,border:'1px solid var(--border)'}}>
                    <div style={{width:38,height:38,borderRadius:'50%',background:c.bg,color:c.color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:800}}>
                      {ini(emp.prenom, emp.nom)}
                    </div>
                    <div style={{flex:1}}>
                      <div style={{fontSize:14,fontWeight:700}}>{emp.prenom} {emp.nom}</div>
                      <div style={{fontSize:12,color:'var(--text2)'}}>{present ? `Présent depuis ${p.heure_arrivee.slice(0,5)}` : 'Absent / non pointé'}</div>
                    </div>
                    <div style={{display:'flex',gap:6}}>
                      {!present
                        ? <button onClick={() => forceArrivee(emp.id)} style={{padding:'7px 12px',borderRadius:9,border:'1px solid var(--green-border)',background:'var(--green-bg)',color:'#1a6b35',fontSize:12,fontWeight:700}}>Forcer arrivée</button>
                        : <button onClick={() => forceDepart(emp.id)} style={{padding:'7px 12px',borderRadius:9,border:'1px solid rgba(255,59,48,.2)',background:'var(--red-bg)',color:'var(--red)',fontSize:12,fontWeight:700}}>Forcer départ</button>
                      }
                    </div>
                  </div>
                )
            ...
            <div style={{padding:'16px 24px',borderTop:'1px solid var(--border)',display:'flex',gap:8}}>
              <button onClick={() => setShowAdmin(false)} style={{flex:1,height:42,borderRadius:12,border:'none',background:'var(--accent)',color:'white',fontSize:13,fontWeight:700}}>Terminer</button>
            </div>
          </div>
        </div>
      )}
