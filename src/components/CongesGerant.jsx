...
export default function CongesGerant({restaurant,employes,showToast,onReloadEmployes}){
  ...
  async function loadAll(){await Promise.all([loadConges(),loadSoldes(),loadPointagesAujourdhui()])}
  ...
  async function loadPointagesAujourdhui(){
    const d=await api.get(`/pointages?restaurant_id=${restaurant.id}&date=${todayStr()}`)
    setPointagesAujourdhui(d||[])
  }
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          {[{id:'demandes',l:'Demandes'},{id:'soldes',l:'Soldes'}].map(t=>(
            <button key={t.id} onClick={()=>setPage(t.id)} style={{
              ...
              {t.id==='demandes'&&enAttente.length>0&&(
                <span style={{position:'absolute',top:4,right:4,minWidth:15,height:15,borderRadius:8,
      ...
      {page==='demandes'&&(<>
        ...
        {/* Bannière absents aujourd'hui */}
        {absentsAujourdhui.length>0&&(
          <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:14,overflow:'hidden'}}>
            <div style={{padding:'10px 14px',borderBottom:'1px solid var(--border)',fontSize:11,fontWeight:700,color:'var(--text2)'}}>
              ABSENTS AUJOURD'HUI
            </div>
            {absentsAujourdhui.map((c,i)=>{
              const type=TYPES[c.type]||TYPES.autre
              return(
                <div key={c.id} style={{
                  display:'flex',alignItems:'center',gap:10,padding:'11px 14px',
                  borderBottom:i<absentsAujourdhui.length-1?'1px solid var(--border)':'none'}}>
                  <div style={{width:32,height:32,borderRadius:'50%',background:type.bg,color:type.c,
                    display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,flexShrink:0}}>
                    {ini(c.prenom,c.nom)}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:700}}>{c.prenom} {c.nom}</div>
                    <div style={{fontSize:11,color:'var(--text2)',marginTop:1}}>
                      {type.emoji} {type.l} · retour le {fmtD(c.date_fin)}
                    </div>
                  </div>
                  <span style={{fontSize:10,fontWeight:700,padding:'2px 9px',borderRadius:20,
                    background:type.bg,color:type.c,border:`1px solid ${type.bc}`,flexShrink:0,whiteSpace:'nowrap'}}>
                    Absent
                  </span>
                </div>
              )
              ...
              {enAttente.map(c=>{
                ...
                return(
                  <div key={c.id} style={{
                    ...
                    border:`1.5px solid ${isOpen?'#E11D48':type.bc}`,
                    overflow:'hidden',transition:'border-color .2s'}}>
                    <div onClick={()=>{setExpanded(isOpen?null:c.id);setCommentaire('')}}
                      style={{padding:'15px 16px',cursor:'pointer',display:'flex',alignItems:'center',gap:12}}>
                      <div style={{width:40,height:40,borderRadius:'50%',background:'#fff1f3',color:'#E11D48',
                        display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:800,
                        flexShrink:0,border:'1.5px solid #fecdd3'}}>
                        {ini(c.prenom,c.nom)}
                      </div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:14,fontWeight:700,marginBottom:5}}>{c.prenom} {c.nom}</div>
                        <div style={{display:'flex',gap:7,alignItems:'center',flexWrap:'wrap'}}>
                          ...
                          <span style={{fontSize:12,color:'var(--text2)'}}>
                            {fmtD(c.date_debut)} → {fmtD(c.date_fin)} · <strong>{jours}j</strong>
                          </span>
                        </div>
                      </div>
                      <span style={{fontSize:16,color:'var(--text3)',transform:isOpen?'rotate(180deg)':'none',transition:'transform .2s',flexShrink:0}}>⌃</span>
                    </div>

                    {isOpen&&(
                      <div style={{borderTop:`1px solid ${type.bc}`,background:type.bg,padding:'16px',display:'flex',flexDirection:'column',gap:12}}>
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                          <div style={{background:'white',borderRadius:11,padding:'12px 14px',border:`1px solid ${type.bc}`}}>
                            <div style={{fontSize:10,fontWeight:700,color:'var(--text3)',marginBottom:5}}>PÉRIODE</div>
                            ...
                            <div style={{fontSize:13,fontWeight:800,color:type.c,marginTop:6}}>{jours} jour{jours>1?'s':''}</div>
                          </div>
                          <div style={{background:'white',borderRadius:11,padding:'12px 14px',border:`1px solid ${type.bc}`}}>
                            <div style={{fontSize:10,fontWeight:700,color:'var(--text3)',marginBottom:5}}>SOLDE</div>
                            {c.type==='conge_paye'&&<><div style={{fontSize:24,fontWeight:900,color:cpS>=jours?'#16a34a':'#dc2626'}}>{cpS}j</div><div style={{fontSize:10,color:'var(--text2)'}}>restants / {solde?.conges_total||25}j{cpS<jours&&<span style={{color:'#dc2626',fontWeight:700}}> ⚠️</span>}</div></>}
                            {c.type==='rtt'&&<><div style={{fontSize:24,fontWeight:900,color:'#7c3aed'}}>{rttS}j</div><div style={{fontSize:10,color:'var(--text2)'}}>RTT restants</div></>}
                            {c.type==='conges_reportes'&&<><div style={{fontSize:24,fontWeight:900,color:'#2563EB'}}>{repS}j</div><div style={{fontSize:10,color:'var(--text2)'}}>reportés restants</div></>}
                            {(c.type==='maladie'||c.type==='sans_solde'||c.type==='autre')&&<div style={{fontSize:12,color:'var(--text2)',marginTop:4}}>Pas de solde</div>}
                          </div>
                        </div>

                        {c.message&&(
                          <div style={{background:'white',borderRadius:11,padding:'11px 14px',border:`1px solid ${type.bc}`}}>
                            <div style={{fontSize:10,fontWeight:700,color:'var(--text3)',marginBottom:4}}>MESSAGE</div>
                            <div style={{fontSize:13,color:'var(--text)',fontStyle:'italic'}}>"{c.message}"</div>
                          </div>
                        )}

                        <textarea value={commentaire} onChange={e=>setCommentaire(e.target.value)}
                          placeholder="Réponse à l'employé (optionnel)..."
                          style={{...inp,resize:'none',height:66,background:'white',border:`1.5px solid ${type.bc}`}}/>

                        <div style={{display:'grid',gridTemplateColumns:'1fr 2fr',gap:8}}>
                          <button onClick={()=>traiter(c.id,'refuse')} style={{
                            height:48,borderRadius:12,border:'1.5px solid #fecaca',
        ...
        {enAttente.length===0&&(
          <div style={{textAlign:'center',padding:'36px 20px',background:'var(--surface)',
        ...
        <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:14,overflow:'hidden'}}>
          <button onClick={()=>setHistoriqueOpen(o=>!o)} style={{
            ...
            <span style={{fontSize:13,fontWeight:700,color:'var(--text)'}}>
              Historique · {conges.filter(c=>c.statut!=='en_attente').length} demande{conges.filter(c=>c.statut!=='en_attente').length!==1?'s':''}
            </span>
            ...
            <div style={{borderTop:'1px solid var(--border)',padding:'12px 14px',display:'flex',flexDirection:'column',gap:10}}>
              <div style={{display:'flex',gap:6,overflowX:'auto',paddingBottom:2}}>
                {[{id:'tous',l:'Tous'},{id:'accepte',l:'Acceptés'},{id:'refuse',l:'Refusés'},{id:'annule',l:'Annulés'}].map(f=>(
                  <button key={f.id} onClick={()=>setFiltreHisto(f.id)} style={{
                    flexShrink:0,padding:'5px 12px',borderRadius:20,fontSize:11,cursor:'pointer',
                    fontWeight:filtreHisto===f.id?700:500,
                    border:`1.5px solid ${filtreHisto===f.id?'#E11D48':'var(--border)'}`,
                    background:filtreHisto===f.id?'#E11D48':'var(--surface)',
                    color:filtreHisto===f.id?'white':'var(--text2)',transition:'all .15s'}}>
                    {f.l}
                  </button>
                ))}
              </div>
              {historique.length===0?(
                ...
                const jours=nbJ(c.date_debut,c.date_fin)
                return(
                  <div key={c.id} style={{display:'flex',alignItems:'center',gap:10,padding:'11px 12px',
                    background:'var(--bg)',borderRadius:12,border:'1px solid var(--border)'}}>
                    <div style={{width:32,height:32,borderRadius:'50%',background:'#f5f5f5',color:'#555',
                      display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,flexShrink:0}}>
                      {ini(c.prenom,c.nom)}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
      ...
      {page==='soldes'&&(
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          <div style={{fontSize:12,color:'var(--text2)',padding:'10px 14px',background:'var(--bg)',
            borderRadius:10,border:'1px solid var(--border)'}}>
            💡 Appuyez sur un solde souligné pour le modifier.
          </div>
          {soldesData.map(emp=>{
                ...
                <div style={{padding:'14px 16px',display:'flex',flexDirection:'column',gap:14}}>
                  ...
                  {(emp.rttTotal>0||edRTT)&&(
                    <div style={{borderTop:'1px solid var(--border)',paddingTop:12}}>
                      ...
                      {emp.rttTotal>0&&(
                        <div style={{height:5,background:'var(--bg)',borderRadius:3,overflow:'hidden',border:'1px solid var(--border)'}}>
                          <div style={{height:'100%',background:rttPct>80?'#dc2626':'#7c3aed',width:`${rttPct}%`,borderRadius:3}}/>
                        </div>
                      )}
                  ...
                  {emp.rttTotal===0&&!edRTT&&(
                    <button onClick={()=>{setEditSolde({empId:emp.id,type:'rtt'});setSoldeTmp('0')}}
                      style={{border:'1px dashed var(--border)',background:'transparent',borderRadius:9,padding:'7px',fontSize:11,color:'var(--text3)',cursor:'pointer',width:'100%'}}>
                      + Ajouter des RTT
                    </button>
                  )}
                      ...
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:11,color:'var(--text2)'}}>
                        ...
                        {edRP?(
                          <span style={{display:'inline-flex',gap:3,alignItems:'center'}}>
                            <input type="number" value={soldeTmp} onChange={e=>setSoldeTmp(e.target.value)} min="0" max="365"
                              style={{width:40,padding:'2px 5px',borderRadius:6,border:'1.5px solid #2563EB',fontSize:11,textAlign:'center',outline:'none'}}/>
                          ...
                          <button onClick={()=>{setEditSolde({empId:emp.id,type:'report_plafond'});setSoldeTmp(String(emp.cpReportPlafond))}}
                            style={{border:'none',background:'transparent',color:'#2563EB',fontWeight:700,fontSize:11,cursor:'pointer',textDecoration:'underline'}}>
                        ...
                        {edRE?(
                          <span style={{display:'inline-flex',gap:3,alignItems:'center'}}>
                            <input type="date" value={soldeTmp} onChange={e=>setSoldeTmp(e.target.value)}
                              style={{padding:'2px 5px',borderRadius:6,border:'1.5px solid #2563EB',fontSize:11,outline:'none'}}/>
                          ...
                          <button onClick={()=>{setEditSolde({empId:emp.id,type:'report_expiration'});setSoldeTmp(emp.cpReportExpiration?String(emp.cpReportExpiration).split('T')[0]:'')}}
                            style={{border:'none',background:'transparent',color:'#2563EB',fontWeight:700,fontSize:11,cursor:'pointer',textDecoration:'underline'}}>
                            {emp.cpReportExpiration?new Date(String(emp.cpReportExpiration).split('T')[0]+'T12:00:00').toLocaleDateString('fr-FR',{day:'numeric',month:'short',year:'numeric'}):'Non définie ✏️'}
                          </button>
                        )}
                      </div>
                      <button onClick={()=>resetAnnuel(emp.id,emp.cpTotal,emp.cpPris,emp.cpReportPlafond)}
                        style={{padding:'6px',borderRadius:8,border:'1px solid #bfdbfe',background:'#eff6ff',
                          color:'#2563EB',fontSize:11,fontWeight:700,cursor:'pointer',marginTop:2}}>
                        Reset annuel
                      </button>
                    </div>
                  </div>
      ...
      {/* MODAL EXPORT */}
      {exportModal&&(
        <div onClick={()=>setExportModal(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.35)',
          backdropFilter:'blur(6px)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:200,padding:'0 0 0 0'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'var(--surface)',borderRadius:'20px 20px 0 0',
            padding:24,width:'100%',maxWidth:480,boxShadow:'0 -4px 40px rgba(0,0,0,.2)'}}>
            ...
            <div style={{display:'flex',gap:6,marginBottom:16}}>
              {[{l:'Ce mois',d:new Date(new Date().getFullYear(),new Date().getMonth(),1).toISOString().split('T')[0],f:new Date(new Date().getFullYear(),new Date().getMonth()+1,0).toISOString().split('T')[0]},{l:'Cette année',d:`${new Date().getFullYear()}-01-01`,f:`${new Date().getFullYear()}-12-31`}].map(p=>(
                <button key={p.l} onClick={()=>setExportForm(f=>({...f,debut:p.d,fin:p.f}))}
                  style={{flex:1,padding:'8px',borderRadius:9,border:'1px solid var(--border)',background:'var(--bg)',fontSize:12,fontWeight:600,cursor:'pointer',color:'var(--text2)'}}>
                  {p.l}
                </button>
              ))}
            </div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>setExportModal(false)}
                style={{flex:1,height:46,borderRadius:11,border:'1px solid var(--border)',background:'var(--bg)',color:'var(--text2)',fontSize:13,fontWeight:700,cursor:'pointer'}}>
                Annuler
              </button>
              <button onClick={()=>{doExportPDF(conges,restaurant,employes,exportForm.emp||null,exportForm.debut||null,exportForm.fin||null);setExportModal(false)}}
                style={{flex:2,height:46,borderRadius:11,border:'none',background:'#16a34a',color:'white',fontSize:13,fontWeight:700,cursor:'pointer'}}>
                📄 Générer
              </button>
            </div>
          </div>
